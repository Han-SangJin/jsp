...
public class YogurtPocessor implements ItemProcessor<CycleVo,List<DailyVo>> {

	private static final Logger logger = LoggerFactory.getLogger(YogurtPocessor.class);
	
	// jobLauncher를 실행하면서 두번째 인자로 넣어준 jobParameter 값을 SPEL을 통해 주입
	// 단 jobParameters에 접근하기 위해서는 해당 스프링 빈의 scope를 step 으로 지정해야 한다. 
	...
	public List<DailyVo> process(CycleVo item) throws Exception {
		...
		Calendar calendar = Calendar.getInstance();								// 202011
		calendar.set(Calendar.YEAR, Integer.parseInt(ym.substring(0, 4))); 		// 2020
		calendar.set(Calendar.MONTH, Integer.parseInt(ym.substring(4))-1); 		// 11     캘린더 클래스는 0부터 시작, 0 -> 1월  .. 11 -> 12월
		calendar.set(Calendar.DAY_OF_MONTH, calendar.getActualMaximum(calendar.DAY_OF_MONTH));	// 30, 31 	월의 마지막 일자.
		...
		int i = 0;
		while(endTimeInt >= dtInt) {		// 캘린더 값이 2020년 11월 30일 보다 작거나 같다
			// 캘린더 날짜가 item의 애음일과 같을 때만 dailyVo를 생성
		...
		logger.debug("YogurtPocessor - dailyVoList , i: {}, {}", dailyVoList, i);
		return dailyVoList;
	}
