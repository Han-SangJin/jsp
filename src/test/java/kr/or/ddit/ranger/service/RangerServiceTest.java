...
public class RangerServiceTest {
	...
	// 해당 메소드가 Test대상임을 알려주는 어노테이션 
	// 일반적으로 메소드 단위로 테스트 메소드를 생성
	// 하나의 메소드도 시나리오에 따라서는 여러개의 테스트 메소드로 생성할 수도 있다.
	// ex: 회원 등록 dao 메소드를 생성시 
	//		정상적인 값을 입력해서 db 테이블에 데이터가 정상적으로 입력되는 케이스
	//		비 정상적인 값(회원id 중복) 을 입력하여 db 테이블에 데이터가 입력되지 않는 실패 케이스
